 ...
 *       int col   - 列数
 *****************************************************************************/
m_Matrix::m_Matrix(int row, int col)
{
	Init(row, col);
}

void m_Matrix::Init(int row, int col)
{
	int i;	
 ...
 *****************************************************************************/
m_Matrix::m_Matrix(int row, int col, double mtx[])
{
	Init(row, col, mtx);
 ...
 *       double mtx[] - row*col数组
 *****************************************************************************/
void m_Matrix::Init(int row, int col, double mtx[])
{
	int i;
 ...
 *****************************************************************************/
m_Matrix::m_Matrix(const m_Matrix &obj)
{
	Init(obj);

}
void m_Matrix::Init(const m_Matrix &obj)
{
	int i;
 ...
 *****************************************************************************/
double m_Matrix::Get(const int i, const int j)const
{	
 ...
 * 功能：修改矩阵元素
 *****************************************************************************/
void m_Matrix::Set(const int i, const int j, const double e)
{
 ...
 * 功能：等号
 *****************************************************************************/
m_Matrix &m_Matrix::operator= (const m_Matrix &obj)
{                
 ...
 * 功能：正号
 *****************************************************************************/
m_Matrix m_Matrix::operator+ ()const
{
	return *this;
}   
 ...
 * 功能：矩阵加法
 *****************************************************************************/
m_Matrix operator+ (const m_Matrix &A, const m_Matrix &B)
{                
    m_Matrix AB(A.Row, A.Col);
...
{                
    ...
    m_Matrix AB(A.Row, B.Col); // AB用于保存乘积
 ...
 * 返回：Matrix - 相乘结果矩阵
 *****************************************************************************/
m_Matrix operator* (const double &a, const m_Matrix &B)
{
    m_Matrix aB(B);
    for (int i=0; i<aB.Row; i++)
	{
 ...
 * 返回：Matrix* B - 矩阵B
 *****************************************************************************/
m_Matrix Trv(const m_Matrix &A)
{
    m_Matrix B(A.Col, A.Row);
 ...
 * 返回：int - 0正常，其他错误
 *****************************************************************************/                             
int Inv(m_Matrix* in, m_Matrix* B)
{
	int i, j, k;
	int ind;
	double max;
...
int Inv_Trans(m_Matrix* in, m_Matrix* B)
{
	int i,j;
	double temp;
...
// 求解两向量角度
double v_angle(IN const Vector &A, IN const Vector &B)
{
	double v_angle;
	double temp;
	temp = A.X * B.X + A.Y * B.Y + A.Z * B.Z;
	double p_length1 = sqrt( A.X * A.X + A.Y * A.Y + A.Z * A.Z);
	double p_length2 = sqrt( B.X * B.X + B.Y * B.Y + B.Z * B.Z);
	v_angle = acos(temp / (p_length1 * p_length2));
	return v_angle;
}

//A向量在B向量上的投影(B为单位向量)
Vector vec_projection(IN const Vector &A, IN const Vector &B)
{
	Vector project_vec;
	project_vec = - operator*(A,B) * B;
	return project_vec;
}
